...
exports.createBmiPlan = async (req, res) => {
    try {
        const { planname, short_desc, title, long_desc, highlight_desc, upload_pdf, plan_max, plan_deductible, coverageAmt, fromDate, toDate, total, email, coverage_area, dob, filterdata } = req.body;
...
exports.adminPlanlist = async (req, res) => {
    const { fromDate, toDate } = req.body;

    try {
        let result = await BmiPlan.find({
            $or: [
